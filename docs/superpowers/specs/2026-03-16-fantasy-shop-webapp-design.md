...
- Note: `items.json` (the local dataset) does not contain descriptions. The item detail view for non-scroll items shows metadata only (name, price, rarity, weight, category, requirements). Descriptions are available only for spells/scrolls (from `dnd-data`).

Artifact rarity items appear in the browse catalog but are excluded from shop generation (no template assigns Artifact weight). Artifacts are unique, world-shaping items that shouldn't appear in random shops.
